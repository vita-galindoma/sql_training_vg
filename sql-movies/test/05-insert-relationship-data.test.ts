...
const insertMovieGenres = (
  ...
  return (
    ...
    genres.map(genreName => `(
        ${movieId}, 
        ${genreRows.filter(row => row.genre === genreName)[0].id}
        )`
    ).join(",")    
  );
};
...
const insertMovieActors = (
  ...
  return (
    ...
    actors.map(actor =>         
      `(${movieId}, ${actorRows.filter(x => x.full_name == actor)[0].id})`
    ).join(",")
  ); 
};
...
const insertMovieDirectors = (
  ...
  return (
    ...
    directors.map(director =>         
      `(${movieId}, ${directorRows.filter(x => x.full_name == director)[0].id})`
    ).join(",")
  ); 
};
...
const insertMovieKeywords = (
  ...
  return (
    ...
    keywords.map(keyword =>         
      `(${movieId}, ${keywordRows.filter(x => x.keyword == keyword)[0].id})`
    ).join(",")
  ); 
};
...
const insertMovieProductionCompanies = (
  ...
  return (
    ...
    productionCompanies.map(company =>         
      `(${movieId}, ${productionCompanyRows.filter(x => x.company_name == company)[0].id})`
    ).join(",")
  ); 
};
